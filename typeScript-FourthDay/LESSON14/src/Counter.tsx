...
const Counter = ({ children }: ChildrenType) => {
  ...
  const increment = () => dispatch({ type: REDUCER_ACTİON_TYPE.INCREMENT });
  const decrement = () => dispatch({ type: REDUCER_ACTİON_TYPE.DECREMENT });
  const handleTextInput = (e: ChangeEvent<HTMLInputElement>) => {
    dispatch({ type: REDUCER_ACTİON_TYPE.NEW_INPUT, payload: e.target.value });
  };
